...
const sendCareerApplicationMail = async (name, email, mobile, title, resumePath = null) => {
  console.log('SMTP disabled - Career application email not sent:', { name, email, mobile, title });
  return; // SMTP disabled

  /* console.log('Sending career application mail with params:', { name, email, mobile, title });
  ...
  const subject = "Skill Up Tech Solutions - New Career Application";
  const html = `
      ...
      <div style="background-color: white; padding: 40px; border-radius: 12px; box-shadow: 0 4px 20px rgba(0,0,0,0.08); border-top: 4px solid #f57f17;">
          ...
          <tr>
            <td style="text-align: center; vertical-align: middle;">
              <div style="color: white; padding: 15px 25px; border-radius: 8px; display: inline-block; margin-bottom: 15px;">
                <h1 style="margin: 0; font-size: 24px; font-weight: 600; color: #f57f17;">New Career Application</h1>
              </div>
              <p style="color: #6b7280; margin: 0; font-size: 16px;">A candidate has applied for a position</p>
            </td>
          ...
          <div style="display: flex; align-items: flex-start;">
            ...
            <div>
              ...
              <ul style="margin: 0; color: #000000ff; line-height: 1.6; padding-left: 20px;">
                <li><strong>Review the candidate's application</strong>${resumePath ? ' and resume (attached)' : ''}</li>
                <li><strong>Contact via:</strong> ${email} or ${mobile}</li>
                <li><strong>Position:</strong> ${title}</li>
                <li><strong>Application received:</strong> ${new Date().toLocaleString()}</li>
                <li><strong>Next steps:</strong> Schedule interview if candidate meets requirements</li>
              </ul>
            </div>
          </div>
        </div>
        ...
        <div style="text-align: center; padding-top: 20px;">
          ...
          <p style="color: #000000ff; font-size: 11px; margin: 0; line-height: 1.4;">
            This email was automatically generated when a candidate submitted a career application form.<br>
            Please respond promptly to maintain a positive candidate experience.
          </p>
        </div>
  ...
  const mailOptions = {
    ...
    subject,
    html,
    replyTo: email, // This allows admin to reply directly to the candidate
  };
  ...
  if (resumePath) {
    ...
    mailOptions.attachments = [
      {
        filename: path.basename(resumePath), // Get filename from path
        path: resumePath
      }
    ];
  }
